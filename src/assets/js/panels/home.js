...
class Home {
    ...
    async initLaunch() {
        document.querySelector('.play-btn').addEventListener('click', async() => {
            ...
            launch.on('check', (e) => {
                progressBar.style.display = "block"
                document.querySelector(".text-download").innerHTML = `Проверка ${((DL / totDL) * 100).toFixed(0)}%`
                progressBar.value = DL;
                progressBar.max = totDL;
            ...
            launch.on('data', (e) => {
                ...
                info.innerHTML = `Подготовка к запуску...`
                console.log(e);
            })
    ...
    async initStatusServer() {
        ...
        if (!serverPing.error) {
            ...
            serverMs.innerHTML = `<span class="red">Не в сети</span>`;
        }
    }
